...
// ── Variantes de animación para las tarjetas del catálogo ────────────────────
// Usando opacity + scale suave para evitar layout thrashing.
const CARD_VARIANTS = {
...
export default function Catalog() {
  ...
  // Estado de color del modelo 3D (para el selector híbrido)
  const [modalColor, setModalColor] = useState('#0a192f');
  ...
  // Al seleccionar Tapas padre → abre/cierra acordeón y selecciona "Todas las Tapas" por defecto
  const handleMenuClick = useCallback((item) => {
    ...
    // Resetear tag al cambiar categoría para evitar filtros fantasma
    setActiveTag('All');
  }, []);
  ...
  // Contadores por categoría (estáticos del dataset)
  const catCounts = useMemo(() => {
    ...
    counts['__tapas__all'] = tapasTotal;
    productsData.forEach(p => { counts[p.category] = (counts[p.category] || 0) + 1; });
    return counts;
  }, []);

  // Contadores de tags dinámicos según categoría activa
  ...
  const handleQtyBlur = (product) => {
    setQuantities(prev => {
      const numericVal = parseInt(prev[product.id], 10);
  ...
  const handleAdd = (product, e) => {
    e.stopPropagation();
    const numericVal = parseInt(getQty(product), 10);
    if (!isNaN(numericVal) && numericVal >= product.moq) addToCart(product, numericVal);
  };

  // Abrir modal y resetear color al color primario del producto (o default)
  const openModal = (product) => {
    setSelectedProduct(product);
    setModalQty(Math.max(getQty(product), product.moq));
    const firstColor = product.colores?.[0] ?? '#0a192f';
    setModalColor(firstColor);
  };
    ...
    <div className="min-h-screen bg-slate-50 font-inter text-slate-800">
      ...
      <div className="pt-32 pb-16 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          {/* ── Sidebar filtros: wrapper outer preserva border-radius ── */}
          <aside className="w-full md:w-64 flex-shrink-0">
            {/*
              Dos capas:
              1. Wrapper exterior: border-radius + overflow-hidden (corta el scrollbar)
              2. Inner: overflow-y-auto + fancy-scrollbar (scroll real)
            */}
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm sticky top-32 overflow-hidden"
              style={{ maxHeight: 'calc(100vh - 9rem)' }}>
              {/* Capa inner con scroll real */}
              <div className="overflow-y-auto fancy-scrollbar h-full p-6"
                style={{ maxHeight: 'calc(100vh - 9rem)' }}>
                    ...
                    {['All', ...new Set(productsData.flatMap(p => p.tags))].map(tag => {
                      ...
                      return (
                        <button key={tag} onClick={() => setActiveTag(tag)}
                          ...
                          {tag === 'All' ? 'Todos' : tag}
                          <span className={`ml-1 font-normal ${isActive ? 'text-cyan-600' : 'text-slate-400'}`}>
                            ({tag === 'All' ? tagCounts['All'] : count})
                          </span>
                        </button>
                      );
                    })}
          ...
          {/* ── Product grid: LayoutGroup coordina animaciones de layout ── */}
          <div className="flex-1">
            <LayoutGroup id="catalog-grid">
              {/* mode="popLayout" hace que los elementos que salen no empujen a los demás */}
                ...
                >
                  {filteredProducts.map(product => (
                    <motion.div
                      ...
                      className="bg-[#fcfdfd] cursor-pointer rounded-3xl border border-slate-200 hover:border-cyan-500 hover:shadow-xl transition-colors flex flex-col overflow-hidden group"
                    >
                      {/* Image */}
                      <div className="relative bg-white p-6 flex items-center justify-center border-b border-slate-100">
                        ...
                        {/* Badge 3D si el producto tiene modelo */}
                        {product.modelo3D && (
                          <span className="absolute top-3 left-3 px-2 py-0.5 text-[10px] font-bold uppercase rounded-full bg-cyan-600 text-white flex items-center gap-1 shadow-sm">
                            <FiBox className="text-[10px]" /> 3D
                          </span>
                        )}
                      </div>
      ...
      {/* ── Modal de Producto con soporte 3D ── */}
      <AnimatePresence>
        {selectedProduct && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              {/* ── Lado Izquierdo: Imagen O Modelo 3D ── */}
              <div className="w-full md:w-1/2 bg-[#f8fafc] flex flex-col border-b md:border-b-0 md:border-r border-slate-200">
                {selectedProduct.modelo3D ? (
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    {/* ── Selector de color híbrido: burbujas predefinidas + input libre ── */}
                    <div className="px-6 pb-6 pt-2">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1.5">
                        <HiColorSwatch className="text-sm text-cyan-600" /> Color del Modelo
                      </p>
                      <div className="flex flex-wrap gap-2 items-center">
                        {/* Burbujas de colores predefinidos del producto */}
                        {(selectedProduct.colores || ['#0a192f', '#06b6d4', '#eba014', '#ef4444', '#10b981']).map(hex => (
                          <button
                            key={hex}
                            ...
                            className={`w-7 h-7 rounded-full border-2 transition-all shadow-sm ${
                              modalColor === hex
                                ? 'border-cyan-500 scale-110 shadow-cyan-200'
                        ...
                        {/* Input de color libre (burbuja especial con icono de gradiente) */}
                        <div className="relative w-7 h-7 rounded-full border-2 border-dashed border-slate-300 hover:border-cyan-400 transition-all overflow-hidden"
              ...
              {/* ── Lado Derecho: Datos y Carrito ── */}
              <div className="w-full md:w-1/2 p-8 md:p-10 flex flex-col overflow-y-auto fancy-scrollbar">
                {/* Encabezado */}
                ...
                <p className="text-slate-500 text-sm mb-6 leading-relaxed">
                  Fabricado con resinas de grado industrial y diseñado para una alta compatibilidad con el mercado.
                  Proporciona un sello seguro y cuenta con aplicaciones múltiples que cumplen los estándares de fabricación.
                </p>
                ...
                <div className="mt-auto">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 mb-2">
                    <input
                      ...
                      className={`w-full sm:w-1/3 bg-white border-2 rounded-xl px-4 py-3 text-lg font-bold text-center focus:outline-none transition-colors ${
                        (parseInt(modalQty, 10) < selectedProduct.moq || modalQty === '')
                          ? 'border-red-500 text-red-600 focus:border-red-600'
                          : 'border-slate-200 text-[#0a192f] focus:border-cyan-500'
                      }`}
                    />
                  ...
                  <button
                    ...
                    className={`w-full py-4 text-white rounded-xl font-bold text-lg flex items-center justify-center gap-3 transition-all shadow-xl ${
                      (parseInt(modalQty, 10) < selectedProduct.moq || modalQty === '')
                        ? 'bg-slate-400 cursor-not-allowed'
                        : 'bg-[#0a192f] hover:bg-black hover:-translate-y-0.5'
                    }`}
                  >
